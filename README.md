...
| `azalea export [-c/--client]` | Export a `.mrpack` archive to `dist/` (optionally client-side only). Client presets: place `<preset-name>.txt` files in `overrides/presets` — each file's contents become `overrides/options.txt` inside the exported preset .mrpack. |
